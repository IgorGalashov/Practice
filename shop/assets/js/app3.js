const btn_cena = document.querySelector('.btn_cena');
let arr = localStorage.getItem('111');
 arr = arr.split(',');
btn_cena.addEventListener('click',function(){
   let id = document.querySelector('.id');
    arr.push(id.textContent);
...
const in_bask = document.querySelectorAll(' .btn_cena');
for(let i=0; i<arr.length; i++)
        if (id.textContent == arr[i])
            btn_cena.style.display = 'none';
